...
def register_callbacks(app):
   ...
   def update_dashboard(year_range, region, activity, season):
       ...
       map_fig = go.Figure(go.Scattermapbox(
           ...
           text=df.apply(
               lambda x: f"Location: {x['Location']}<br>Date: {x['Year']}<br>Activity: {x['Victim.activity']}<br>Species: {x['Shark.common.name']}<br>Outcome: {x['Victim.injury']}",
               axis=1),
           hoverinfo='text'
       ))
       ...
       most_common_season = season_counts.idxmax() if len(season_counts) > 0 else "N/A"
       ...
       insights = html.Div([
           html.P(f"Peak activity recorded in {peak_year} with most incidents occurring in {most_common_season}."),
